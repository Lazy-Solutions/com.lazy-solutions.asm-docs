...
| `SceneOperation CloseAll(IEnumerable<Scene> scenes, Scene loadingScene)` | Closes the `scenes`. |
...
| `SceneOperation OpenAll(IEnumerable<Scene> scenes, Scene loadingScene)` | Opens the `scenes`. |
